...
plt.title('Class distribution before oversample - SMOTETomek')
plt.xlabel('Class')
plt.ylabel('Count')
plt.show()
plt.clf()
...
# X_res.to_csv("SMOTETomek-report/SMOTETomek-BreastCancerWisconsin(Diagnostic).csv", index=False)

class_counts = X_res["diagnosis"].value_counts()

class_counts.plot(kind='bar')
plt.title('Class distribution after oversample - SMOTETomek')
plt.xlabel('Class')
plt.ylabel('Count')
plt.show()
plt.clf()
