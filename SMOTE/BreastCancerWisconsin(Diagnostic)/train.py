...
# Load the dataset
title = "4 model on BreastCancerWisconsin(Diagnostic) SMOTE"
doc = Document()
doc.add_heading(title, level=1)

df_src = "SMOTE-report/SMOTE-BreastCancerWisconsin(Diagnostic).csv"
...
plt.title("RF confusion matrix - SMOTE")
...
disp.plot(cmap=plt.cm.Blues)
plt.title("ADABoost confusion matrix - SMOTE")
plt.xticks(rotation=40, fontsize=5)
plt.yticks(fontsize=5)
plt.show()
plt.clf()
...
disp.plot(cmap=plt.cm.Blues)
plt.title("LGBM confusion matrix - SMOTE")
plt.xticks(rotation=40, fontsize=5)
plt.yticks(fontsize=5)
plt.show()
plt.clf()
...
doc.save(f"SMOTE-report/{title}.docx")
