...
plt.title('Class distribution before oversample - SMOTENC')
plt.xlabel('Class')
plt.ylabel('Count')
plt.show()
plt.clf()
...
sm = SMOTENC(random_state=42, categorical_features=["id"])
...
X_res = X_res.drop("id", axis=1)
X_res.to_csv("SMOTENC-report/SMOTENC-BreastCancerWisconsin(Diagnostic).csv", index=False)

class_counts = X_res["diagnosis"].value_counts()

class_counts.plot(kind='bar')
plt.title('Class distribution after oversample - SMOTENC')
plt.xlabel('Class')
plt.ylabel('Count')
plt.show()
plt.clf()
